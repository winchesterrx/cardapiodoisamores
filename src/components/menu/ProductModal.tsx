                ...
                <>
                  ...
                  {availableAddons.length > 0 && (
                    <div className="mt-4">
                            ...
                            <p className="text-xs text-muted-foreground mb-4"><strong>Passo 2:</strong> Escolha seus 5 Adicionais gratuitos!</p>
                              ...
                              {availableAddons.filter(a => a.type === 'creme').length > 0 && (
                                <div className="mb-4 mt-2">
                                  ...
                                  <div className="space-y-1.5 mt-2">
                                    {(() => {
                                      let freeCremesUsed = 0;
                                      return availableAddons.filter(a => a.type === 'creme').map((addon) => {
                                        const qty = addonQuantities[addon.id] || 0;
                                        
                                        let isPaidCreme = false;
                                        let displayLabelCreme = "";

                                        if (qty > 0) {
                                          let freeQtyForThis = 0;
                                          if (freeCremesUsed < 1) {
                                            freeQtyForThis = Math.min(qty, 1 - freeCremesUsed);
                                            freeCremesUsed += freeQtyForThis;
                                          }
                                          let paidQtyForThis = qty - freeQtyForThis;

                                          if (freeQtyForThis > 0 && paidQtyForThis > 0) {
                                            isPaidCreme = true;
                              ...
                              {availableAddons.filter(a => a.type === 'adicional' || a.type === 'normal' || a.type === 'comum').length > 0 && (
                                <div className="mb-4">
                                  ...
                                  <div className="space-y-1.5 mt-2">
                                    {(() => {
                                      let freeAddUsed = 0;
                                      return availableAddons.filter(a => a.type === 'adicional' || a.type === 'normal' || a.type === 'comum').map((addon) => {
                                        const qty = addonQuantities[addon.id] || 0;
                                        ...
                                        if (qty > 0) {
                                          let freeQtyForThis = 0;
                                          if (freeAddUsed < 5) {
                                            freeQtyForThis = Math.min(qty, 5 - freeAddUsed);
                                            freeAddUsed += freeQtyForThis;
                                          }
                                          let paidQtyForThis = qty - freeQtyForThis;
                      ...
                      {!(product.isBarca || (product.categoryName && product.categoryName.toLowerCase().includes("barca"))) && !(product.isCombo || (product.categoryName && product.categoryName.toLowerCase().includes("combinado"))) && availableAddons.some(a => !isAddonFree(a.id)) && (
                        <div>
